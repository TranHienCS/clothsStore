 ...
 */
const jwtHelper = require("../helpers/helperjwt.js");
...
let login = async (req, res) => {
  try {
    ...
    let isValidLogin = await User.findOne({email:req.body.email},(err,docs)=>docs)
    if(!isValidLogin){
      return res.json({status:false,message:'Email dose not exist'})
    }
    if(!(await bcrypt.compare(req.body.password,isValidLogin.password))){
      return res.json({status:false,message:'Password is not correct'})
    }
    ...
    return res.status(200).json({status:true,accessToken,userData: isValidLogin});
  } catch (error) {
...
let handleRegister =async (req,res)=>{
  ...
  try{
      ...
      await user.save();
      res.status(200).json({status:'true',result:'Create account success'})
  } catch(error){
      console.log(error);
  }
}
